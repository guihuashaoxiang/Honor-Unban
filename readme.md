...
> **⚠️ 重要：模板图片必须用你自己的屏幕重新截取，不要直接套用本仓库自带的图片。**
> 不同显示器分辨率、系统缩放比例（DPI）会让界面像素存在差异，直接使用别人的模板会因匹配不上而出现“找不到选项”的问题。仓库自带的 `templates/` 仅供命名参考，实际使用前请全部按下面的规则自行替换。

尽管脚本不再依赖图像识别题目内容，但仍需要模板图片来 **定位选项和提交按钮的点击位置**。

1.  确保 `templates` 文件夹存在。
2.  对你的答题界面的 **A, B, C, D 选项图标** 和 **提交按钮** 进行截图。截图越小、越清晰越好。
3.  将截图按以下命名规则保存到 `templates` 文件夹：
    -   `option_A_1.png`, `option_B_1.png`, ..., `submit_button.png`
    -   **关于 `_1` / `_2` 后缀（两种题型布局）**: 纯文字题与三视图（图片）题的 ABCD 选项位置/排布不同，因此同一选项需要两张模板分别匹配两种布局。命名约定为：`_1` 后缀对应 **文字题型** 的布局，`_2` 后缀对应 **三视图（图片）题型** 的布局，例如 `option_A_1.png`、`option_A_2.png`。请对两种题型各截一组，确保都覆盖 A/B/C/D 四个选项。

> 截图时建议用系统自带的截图工具（Win+Shift+S）框选尽量小且紧贴图标边缘的区域，并保持答题界面处于 100% 缩放状态，以提高模板匹配的稳定性。
